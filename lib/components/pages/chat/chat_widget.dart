...
class _ChatWidgetState extends State<ChatWidget> {
  ...
  void initState() {
    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      // Updated on 2024-12-19 16:00 by Gungyun
      try {
        // Generate user id
        _model.userId = functions.generateUserId();
        safeSetState(() {});
        
        // Validate user ID generation
        if (_model.userId == null || _model.userId!.isEmpty) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Generated user ID: ${_model.userId}',
              style: TextStyle(
        ...
        // Check API response
        if (_model.userResponse?.statusCode != 200) {
          throw Exception('Failed to create chat user: ${_model.userResponse?.statusCode}');
        }

        // Extract user key
        _model.userKey = (_model.userResponse?.jsonBody ?? '').toString();
        if (_model.userKey == null || _model.userKey!.isEmpty) {
          throw Exception('Failed to extract user key from response');
        }
        safeSetState(() {});
        
        // Generate conversation id
        _model.conversationId = functions.generateUserId();
        if (_model.conversationId == null || _model.conversationId!.isEmpty) {
          throw Exception('Failed to generate conversation ID');
        }
        safeSetState(() {});
        ...
        if (_model.conversationResponse?.statusCode != 200) {
          throw Exception('Failed to create conversation: ${_model.conversationResponse?.statusCode}');
        }
        ...
        _model.addToMessages(functions.createMessage(
            'Sorry, there was an error initializing the chat. Please try again.',
            'bot',
            getCurrentTimestamp)!);
        safeSetState(() {});
      }
